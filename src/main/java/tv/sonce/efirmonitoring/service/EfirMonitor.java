...
public class EfirMonitor implements Runnable {
    ...
    private static final long MAX_FREEZE_TIME = 7 * 1000L; // ms
    private static final long TIME_UNTIL_REBOOT = 4 * 60 * 60 * 1000L; // Раз в 4 часа отключаемся от IP потока и подключаемся снова
    private static final int MAX_COUNT_OF_DROPPED_FRAMES = 5;    // сколько потерянных кадров
    private static final long MAX_TIME_DROPPED_FRAMES = 5 * 1000L; // за сколько времени мерять потерянные кадры
    private static final long TIME_FOR_SLEEPING_WHEN_FRAMES_DROPPED = 5 * 1000L;
...
//        videoStream = new VideoCapture(0);
    ...
    public void run() {
        ...
        Mat frameCurrent = new Mat(streamer.getHeight(), streamer.getWidth(), CvType.CV_8UC3);
        Mat framePrevious = new Mat(streamer.getHeight(), streamer.getWidth(), CvType.CV_8UC3);
        Mat frameResult = new Mat(streamer.getHeight(), streamer.getWidth(), CvType.CV_8UC3);
        Mat frameTemp = new Mat(streamer.getHeight(), streamer.getWidth(), CvType.CV_8UC3);
        Size size = new Size(3, 3);
        Mat mat = new Mat();
        Scalar RED = new Scalar(0, 0, 255); //BGR
        Scalar GREEN = new Scalar(0, 255, 0);

//        View view = new View(videoStream, frameTemp); //---------------------------------------------------
...
//
                if (!(logoV2Present || logoTraurPresent)) {
                    alarmMessage += getTime() + " Logo is missing! ";
                }
                ...
                Imgproc.GaussianBlur(frameCurrent, frameCurrent, size, 0);

                if (index > 1) {
                    Core.subtract(framePrevious, frameCurrent, frameResult);
                    Imgproc.cvtColor(frameResult, frameResult, Imgproc.COLOR_RGB2GRAY);
                    Imgproc.threshold(frameResult, frameResult, sensivity, 255, Imgproc.THRESH_BINARY);

                    List<MatOfPoint> contours = new ArrayList<>();
                    Imgproc.findContours(frameResult, contours, mat, Imgproc.RETR_LIST, Imgproc.CHAIN_APPROX_SIMPLE);
...
//                            frame.copyTo(frameTemp); //---------------------------------------------------
                    ...
                    if (!foundMovement) {
                        if (currentTime - previousTime > MAX_FREEZE_TIME) {
                            alarmMessage += getTime() + " Video frozen ";
...
//                        view.repaint(); //---------------------------------------------------
                ...
                if (countOfDroppedFrames >= MAX_COUNT_OF_DROPPED_FRAMES) {
                    alarmMessage += getTime() + " Не удалось прочитать кадр с тюнера ";
                    for (Notifier notifier : notifiers) {
                        notifier.sendMessage(alarmMessage);
                    }
                    alarmMessage = "";
